...
The code can be used in two ways:
    1. By importing solve_foregound_background in your code:
        ```
            ...
            ...
            # For scribles input
            alpha = closed_form.with_scribbles(image, scribbles)

            # For trimap input
            alpha = closed_form.with_trimap(image, trimap)

            # For prior with confidence
            alpha = closed_form.with_prior(
    ...
    2. From command line:
        ```
            # Scribbles input
            ./closed_form.py input_image.png -s scribbles_image.png  -o output_alpha.png

            # Trimap input
            ./closed_form.py input_image.png -t scribbles_image.png  -o output_alpha.png
...
def with_prior(image, prior, prior_confidence, consts_map=None):
...
def with_trimap(image, trimap, trimap_confidence=100.0):
    """Apply Closed-Form matting to given image using trimap."""

    assert image.shape[:2] == trimap.shape, (
        'trimap must be 2D matrix with height and width equal '
        'to image.')
    consts_map = (trimap < 0.1) | (trimap > 0.9)
    return with_prior(image, trimap, trimap_confidence * consts_map,
                      consts_map)


def with_scribbles(image, scribbles, scribbles_confidence=100.0):
    ...
    return with_prior(image, prior, scribbles_confidence * consts_map,
                      consts_map)


closed_form = with_trimap
